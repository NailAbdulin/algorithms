...
public class IntegerListImpl implements IntegerList {
    ...
    @Override
    public Integer add(Integer item) {return add(capacity, item);}
    ...
    private void checkNotNull(Integer item) {
        if (Objects.isNull(item)) {
            throw new IllegalArgumentException("Нельзя хранить в списке null");
        }
    }

    private void checkNonNegativeIndex(int index) {
        if (index < 0) {
            throw new IllegalArgumentException("Индекс должен быть отрицательный");
        }
    }
